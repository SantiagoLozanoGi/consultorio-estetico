...
export default function AgendarCalendar({
  ...
  /** Carga bloqueos + citas existentes para la fecha seleccionada */
  const cargarHorasOcupadas = async (fechaISO: string) => {
    setLoadingHoras(true);
    try {
      const [bloqueosRes, citasRes] = await Promise.all([
        api.get<{ ok: boolean; bloqueos: { hora: string }[] }>(
          `/bloqueos-horas?fechaISO=${fechaISO}`
        ),
        api.get<{ ok: boolean; citas: { hora: string; estado: string }[] }>(
      ...
      if (bloqueosRes.ok) {
        bloqueosRes.bloqueos.forEach((b) => ocupadas.add(b.hora));
      }
      if (citasRes.ok) {
        citasRes.citas
          .filter((c) => c.estado !== "cancelada")
          .forEach((c) => ocupadas.add(c.hora));
      }
  ...
  return (
    <div className="w-full max-w-4xl mx-auto grid md:grid-cols-2 gap-6">
      ...
      <div
        ...
        style={{ background: "#FBF7F2", border: "1px solid #E9DED2" }}
      >
        {/* Navegación mes */}
        <div className="d-flex justify-content-between align-items-center mb-4">
          <button
            ...
            style={{ backgroundColor: "#E9DED2", color: PALETTE.text, width: 36, height: 36 }}
          >‹</button>
          <h3 className="fw-bold m-0" style={{ color: PALETTE.text, fontSize: "1.1rem" }}>
            {nombresMes[mes]} {anio}
          </h3>
          <button
        ...
        <div className="d-grid mb-2" style={{ gridTemplateColumns: "repeat(7,1fr)", textAlign: "center" }}>
          {["L","M","X","J","V","S","D"].map((d) => (
            <div key={d} style={{ fontSize: "0.75rem", color: PALETTE.textSoft, fontWeight: 700, paddingBottom: 4 }}>{d}</div>
          ))}
        </div>
            ...
            return (
              <motion.button
                ...
                style={{
                  ...
                  backgroundColor: isSelected ? PALETTE.main : disabled ? "transparent" : "#fff",
                  color: isSelected ? "#fff" : disabled ? "#ccc" : PALETTE.text,
      ...
      >
        <h3 className="fw-bold mb-4 text-center" style={{ color: PALETTE.text, fontSize: "1.1rem" }}>
          {selectedDate ? `Horas disponibles — ${selectedDate}` : "Selecciona un día"}
        </h3>

        {!selectedDate && (
          <p className="text-center" style={{ color: PALETTE.textSoft }}>
        ...
        {selectedDate && loadingHoras && (
          <div className="text-center py-4">
            <div className="spinner-border spinner-border-sm" style={{ color: PALETTE.main }} role="status" />
          </div>
        )}
          ...
          <div className="d-grid" style={{ gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
            {HORAS_BASE.map((h) => {
              ...
              return (
                <motion.button
                  key={h}
                  whileHover={ocupada ? {} : { scale: 1.06 }}
                  whileTap={ocupada ? {} : { scale: 0.96 }}
                  disabled={ocupada}
                  onClick={() => onHoraSelect(h)}
                  className="rounded-pill py-2 px-1 fw-semibold"
                  style={{
                    fontSize: "0.78rem",
                    border: "none",
                    backgroundColor: isSelected
                      ...
                      ? "#F0E8E0"
                      : "#fff",
                    color: isSelected ? "#fff" : ocupada ? "#ccc" : PALETTE.text,
