...
interface Testimonio {
  id: number; nombre: string; texto: string;
  video: string; thumb: string; activo: boolean; destacado: boolean;
}
...
export default function TestimoniosList() {
  ...
  const loadTestimonios = () => {
    api.get<{ ok: boolean; testimonios: Testimonio[] }>("/testimonios")
      .then((res) => { if (res.ok) setTestimonios(res.testimonios); })
      .catch(console.error);
  };
  ...
  const handleGuardar = async () => {
    ...
    try {
      if (modo === "crear") {
        await api.post("/testimonios", { ...form, activo: true, destacado: false });
      } else if (actual) {
        await api.put(`/testimonios/${actual.id}`, form);
      }
      loadTestimonios();
      resetForm();
    } catch (e: any) {
  ...
  const startEditar = (t: Testimonio) => {
    setActual(t);
    setForm({ nombre: t.nombre, texto: t.texto, video: t.video, thumb: t.thumb });
    setModo("editar");
  };
  ...
  return (
    <div>
      ...
      {testimonios.length === 0 ? (
        <p className="text-center py-5" style={{ color: "#8B7060" }}>No hay testimonios aún.</p>
      ) : (
